...
-- safe to re-run. Schema-agnostic for the GL journal: the balanced-posting
-- trigger and grant resolve the table by whichever name it carries —
-- WLT_GL_BATCH on a fresh install from wallet_schema.sql, or WLT_BATCH on a
-- pre-rename upgrade (the rename migration sorts AFTER this one). This lets the
-- documented fresh-install path (schema → SPs → seeds → migrations) succeed,
-- which it previously could not (the file hard-coded the pre-rename name).
--
