...
| # | File | Contents |
|---|------|----------|
| 1 | `schema.sql` | All DDL — tables, sequences, indexes, constraints, functions, procedures, triggers (incl. the balanced-posting `CONSTRAINT TRIGGER` + DEFAULT partitions), views, `pgcrypto`/`uuid-ossp` extensions, and GRANTs. **No data.** |
| 2 | `seed.sql` | Reference / master data only (data-only): `fm_gl_mast` (GL master), `wlt_gl_map` (COA map), `wlt_tran_def` (tran types), `fm_currency`, `wlt_acct_type`. |
