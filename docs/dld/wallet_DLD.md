...
**Changelog**
- v1.6.7 (2026-06-04): **Merchant group lifecycle completed** (US-1.10/1.11/1.12). Add SPs `provision_acct_group` (group row + `SETTLEMENT` account in one TX, using the deferred `fk_group_settlement` per §2.2a — group created cold, `SHARD_COUNT=0`), `post_merchant_deposit` (inbound deposit routing: credits `SETTLEMENT` while cold / a reference-hashed `SHARD` once hot — the settlement-vs-shard branch is in the caller, not `fn_resolve_shard_acct_no`, which stays shard-only), and `rescale_hot_wallet` (upscale 4→8→16 + rebalance by draining shards to settlement via `post_sweep_shard` URGENT). New tran type `MERCHDEP` (CR, contra `109.03.001`). New error codes P0055 `INVALID_GROUP_TYPE` / P0056 `GROUP_ALREADY_EXISTS` / P0057 `GROUP_NOT_ACTIVATED`. Endpoints: `POST /v1/merchant-groups`, `POST /v1/merchant-groups/:group_id/rescale`, `POST /v1/finance/merchant-deposit`. Tests: `wallet_merchant_group_lifecycle_test.sql` (17/17).
