...
- **Endpoint:**
  ...
  2. Chi tiết: `GET /v1/finance/transactions/{tfr_key}` — trả về tất cả các leg (Nợ/Có/Phí/VAT) của một giao dịch.
...
| Finance | GET | `/v1/finance/transactions/{tfr_key}` | `GetTransaction` | Read |
...
3. **Thực thi & Audit:** Khi gọi xuống Core, bắt buộc gửi header audit (mục 1.1):
   ...
   - Thành công → Core trả về `reference`/`tfr_key`; Portal cập nhật `COMPLETED`.
