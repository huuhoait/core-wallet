...
**Current coverage:** the diff trigger fires on `FM_CLIENT_BANKS` (`trg_audit_fm_client_bk`), `FM_CLIENT_KYC` (`trg_audit_fm_kyc`), and — since US-8.5 (2026-06-10) — `FM_CLIENT` (`trg_audit_fm_client`) and `FM_CLIENT_CONTACT` (`trg_audit_fm_client_ct`). The latter two are **`AFTER UPDATE`** only — **UPDATE, not INSERT** (a create has no before→after diff and is already attributed by `created_by` / `created_at`; soft-delete is an `UPDATE status='C'`, so it's covered) — so `update_client` (mutates `FM_CLIENT`) is now fully audited. All `client_no`-bearing client-master tables are therefore covered; `FM_CLIENT_INDVL` / `FM_CLIENT_IDENTIFIERS` no longer exist (US-1.15 folded them into `FM_CLIENT_KYC.extra_data`). When you add a new client-master table or mutating SP, attach `fn_audit_client_change` the same way.
